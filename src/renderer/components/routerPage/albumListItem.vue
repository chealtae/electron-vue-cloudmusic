    ...
    <div class="mian_content">
        <div class='introduce_header'>
            <div class="introduce_img">
                <img :src="albumDeatils.image" alt="" style="height:100%; width:100%">
            </div>
            <div class="introduce_content">
                ...
                <!-- <div class="author" style="height:25px">
                    <img :src="albumDeatils.image" alt="" class="author_img">
                    <span class="author_span" @click="showAuthor">
                        {{albumDeatils.nickname}}
                    </span>
                    <span class="createTime_span">  
                        {{albumDeatils.createtime}}
                    </span>
                </div> -->
                <div class="opreation" style="    margin-top: 10px; margin-bottom: 10px;">
                    <el-row>
                        <el-button type="primary" size="small" round icon="el-icon-caret-right">播放全部</el-button>
                        <el-button v-if="isCollect"  size="small" round icon="l-icon-folder-checked" @click="cancelCollect">已收藏({{albumDeatils.totalcollect}})</el-button>
                        <el-button v-else round size="small" icon="el-icon-folder-add" :disabled="isMine" @click="collect">收藏({{albumDeatils.totalcollect}})</el-button>
                        <!-- <el-button round size="small" icon="el-icon-share">分享({{albumDeatils.totalshare}})</el-button> -->
                        <!-- <el-button round size="small" icon="el-icon-download">全部下载</el-button> -->
                    </el-row>
                </div>
                <div class="label">
                    <span class="label_span" style="float:left">歌手:</span>
                    <span style="color:blue;cursor:pointer" @click="go2Singer(albumDeatils.singer.id)">{{albumDeatils.singer.name}}</span>
                    <!-- <div v-for="item in albumDeatils.label" :key="item.name" style="float:left" >
                        <span class="label_span1" >{{item.name}}</span>
                        <span>/</span>
                    </div> -->
                </div>
                <div class="playtimes">
                    <span>时间:{{albumDeatils.issuedate}} </span>
                </div>
                <!-- <div class="introduction">
                    <span>介绍:{{albumDeatils.introduction}}</span>
...
export default {
    ...
    mounted(){
        ...
        if(this.$route.query){
            ...
            this.$axios.get(`Album/getSongListInfo?listId=${this.listId}`).then((res) => {
                if(res.data.success){
                    this.albumDeatils = res.data.albumDeatils;
                    this.albumDeatils.image = getImgSrc(res.data.albumDeatils.image)
                    
                    let temp = this.albumDeatils.issuedate.split(' ');
                    this.albumDeatils.issuedate = temp[0]; //存入数据库中的时间会精确到小时 需要处理
                    this.tableData = res.data.songList;
                    this.isMine = this.albumDeatils.userid === Number(localStorage.getItem("userId"))
                    console.log(this.isMine,this.albumDeatils.userid, Number(localStorage.getItem("userId")))
                }
            })
        } 
        this.busListener();
    ...
    methods: {
        ...
        getSongListInfo(){
            this.$axios.get(`SongListInfo/getSongListInfo?listId=${this.listId}`).then((res) => {
                if(res.data.success){
                    this.albumDeatils = res.data.albumDeatils;
                    this.albumDeatils.image = getImgSrc(res.data.albumDeatils.image)
                    this.albumDeatils.nickname = res.data.albumDeatils.userinfo.nickname
                    this.tableData = res.data.songList;
                    this.isMine = this.albumDeatils.userid === Number(localStorage.getItem("userId"))
                }
            })
        ...
        judgeCollect(){
            let info = {
                ...
                collectType : 1,//0 歌曲 1 专辑 2 歌单 3动态 4歌手
            }
        ...
        collect(){
            ...
            this.$axios.get(`/Album/addAlbumCollect?albumId=`+this.listId).then(res => {
                if(res.data.success){
                    this.albumDeatils.totalcollect++;
                }
            })
        ...
        cancelCollect(){
            ...
            this.$axios.get(`/Album/subAlbumCollect?albumId=`+this.listId).then(res => {
                if(res.data.success){
                    this.albumDeatils.totalcollect--;
                }
            })
